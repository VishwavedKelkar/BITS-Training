...
type Product = { id: number, image: string, title: string, price: number, quantity: number }
...
const displayProducts = (products: Product[]) => {
    ...
    products.map((element) => {
        ...
        productElement.innerHTML = `<img src= "${element.image}" alt="${element.title}" class = "product-image"> 
                                    <div class="product-details"> 
                                    <h2>${element.title}</h2>
                                    <p class='product-price'>${element.price}</p>
                                    <button class = "add-to-cart-btn" onclick="addToCart(${element.id})" type="button">Add to Cart</button>
                                    </div> `;
        productList.appendChild(productElement); console.log("Child appended to parent");
    })
}
